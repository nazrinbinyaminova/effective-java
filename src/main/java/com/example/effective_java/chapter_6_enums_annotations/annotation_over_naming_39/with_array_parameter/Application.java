...
public class Application {
    public static void main(String[] args) throws ClassNotFoundException {
        int test = 0;
        int passed = 0;
        Class<?> sample = Class.forName("com.example.effective_java.chapter_6_enums_annotations.annotation_over_naming_39.with_array_parameter.SampleClass");
        ...
        for (Method method : methods) {
            if (method.isAnnotationPresent(ExceptionTest.class)) {
                ...
                try {
                    ...
                    Class<? extends Throwable>[] mExps = method.getAnnotation(ExceptionTest.class).value();
                    for (Class<? extends Throwable> ex : mExps) {
                        if (ex.isInstance(exp)) {
                            passed++;
                        }
                    }
                } catch (Exception e) {
